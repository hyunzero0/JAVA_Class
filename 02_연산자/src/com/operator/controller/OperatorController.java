...
public class OperatorController {
	...
	public void singleOp() {
		...
		//기능호출했을 때 부정하기
		//String의 값을 비교할 때는 equals()를 이용해야함.
		//이름이 유병승이니?
		String name = "유병승";
		...
		//다른연산자와 같이 연산실행했을 때 순서변경됨.
		//int testNum = ++count+10;
		//전위연산 : 다른연산자와 같이 있을 때 먼저 증가, 감소연산을 실행하고 다른 연산을 실행
		int testNum = count+++10;
		//후위연산 : 다른 연산과 같이 있을 때 다른 연산을 먼저 실행하고 나중에 증가, 감소연산을 실행
		System.out.println(testNum);
		System.out.println("count : " + count); //후위연산 후 count에 증가 실행한 값
		
		testNum = 0;
		count = 0;
		//testNum = --count;
		testNum = count--; //후위연산이라 0으로 출력
		System.out.println(testNum + " " + count);
		
		//반복문을 사용할 때 많이 사용
		//for(int i=0; i<10; i++) {
		//특정로직의 실행 횟수를 확인할 때 많이 사용
	...
	public void calculatorOp() {
		...
		//곱하기, 나누기
		System.out.println("*연산 : " + su * su2);
		System.out.println("/연산 : " + su / (double)su2); //나누기할 때는 실수가 나오는 경우가 많음 자료형 확인.
	...
	public void calc() {
		...
		//더한 결과, 뺀 결과, 곱한 결과, 나눈 결과를 출력하는 기능을 구현
		//더한 결과 : 00 
		//뺀 결과 : 00
		//곱한 결과 : 00
		//나눈 결과 : 00.00
		...
		System.out.println("나눈 결과 : " + num1 / (double)num2);
	...
	public void compareOp() {
		...
		System.out.println(dnum != dnum1);
		dnum = 13;
		System.out.println(dnum == num); //num이 자동형변환 돼서 double로 변환됨
		...
		System.out.println("A!=a : " + (ch != ch1));
		int chInt = 65;
		System.out.println(ch == chInt); //ch이 자동형변환 돼서 int로 변환됨
		
		//문자열에 대한 동등비교
		//String(클래스) 동등비교할 때 == 연산을 사용할 수 없다!
...
//		System.out.println("유병승".equals(inputName)); //앞뒤에 모두 문자열만 오면 비교 가능
//		//불일치비교
//		System.out.println(!"유병승".equals(inputName));
		...
		String str = "30";
		String str2 = "40";
		//System.out.println(str < str2); 대소비교 불가능
	...
	public void testStr() {
		...
		String id = sc.next();
		System.out.println(id.equals("admin")); //(id + " : " + id.equals("admin")
	...
	public void check() {
		...
		int age = sc.nextInt();
		System.out.println(age > 19); //(age + " : " + (age >= 20)
	}
	
	public void logicOp() {
		//논리연산하기
		//논리연산 : 2개 이상의 진위형을 연결할 때 사용하는 연산
		// && : and(그리고) 두 개의 진위형이 모두 true일 때 true 반환
		// || : or(또는) 두 개의 진위형 중 한 개가 true일 때 true 반환
		// id가 admin이고, password가 123면 true 아니면 false
		String id = "admin";
		String password = "1111";
		System.out.println(id.equals("admin") && password.equals("1234"));
		
		//입력받은 나이가 19살 이상이고 경기도에 살면 true 출력 아니면 false 출력
		Scanner sc = new Scanner(System.in);
		System.out.print("나이 : ");;
		int age = sc.nextInt();
		sc.nextLine();
...
//		System.out.println(('A' <= ch && ch <= 'Z') || ('a' <= ch && ch <= 'z')); 
	...
	public void calcGrade() {
		//수학, 영어, 국어, 코딩점수를 입력받고 합계와 평균을 출력하는 기능을 구현하기
		//*변수는 한개만 사용해서 구현할 것!
		Scanner sc = new Scanner(System.in);
		...
		System.out.println("평균 : " + score / 4.0); //나누기는 실수 나올 수 있으므로 실수로 변환
		
	}
	
	//사용자에게 메세지를 4개 입력받고 입력받은 메세지를 한 번에 출력하는 기능 만들기
	//public void inputMsg();
	
	public void inputMsg() {
		Scanner sc = new Scanner(System.in);
		
		System.out.print("메세지1 : ");
		String msg = sc.nextLine()+"\n"; //+"\n" = 개행
		System.out.print("메세지2 : ");
		msg += sc.nextLine()+"\n";
		System.out.print("메세지3 : ");
		msg += sc.nextLine()+"\n";
		System.out.print("메세지4 : ");
		msg += sc.nextLine()+"\n";
	...
	public void thirdOp() {
		//삼항연산자에 대해 알아보자.
		//조건식(true, false)나오는 연산, 기능(메소드) ? 조건식, 기능이 true일 때 실행할 로직 : 조건식, 기능이 false일 때 실행할 로직
...
//		
//		String result2 = (id.equals("admin") && password.equals("1234")) ? "로그인 성공" : "로그인 실패";
//		
...
//		String tag = "<input type = 'checkbox' name = 'hobby' "+(id.equals("admin")?"checked":"")+">";
		...
		//나이와 성별을 입력받아 나이가 성년이고 남자면 멋진 남자네요! 출력
		//나이가 성년이고 여자면 예쁘시네요! 출력
		//나이가 미성년이면 공부하세요! 출력
		System.out.print("나이 : ");
		int age = sc.nextInt();
		System.out.print("성별(M/F) : ");
		char gender = sc.next().charAt(0);
		String result = age > 19 ? gender == 'M' ? "멋진 남자네요!" : "예쁘시네요!" : "공부하세요!";
